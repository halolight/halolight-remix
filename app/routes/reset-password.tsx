...
// 密码规则
const passwordRules = [
  { label: "至少 6 位", test: (p: string) => p.length >= 6 },
...
export async function action({ request }: { request: Request }) {
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : "重置失败" }
  }
}

export function meta() {
  return generateMeta("/reset-password")
}
...
export default function ResetPassword() {
  ...
  const token = searchParams.get("token") || "mock-reset-token-12345" // 演示用默认token
    ...
    <AuthShell
      ...
      right={
        ...
        >
          ...
          >
            <div className="inline-flex items-center gap-3 mb-3 px-6 py-3 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 shadow-xl">
          ...
          <Card className="border border-border/50 shadow-2xl backdrop-blur-xl bg-card/80 overflow-hidden">
            {!isSuccess ? (
              <>
                <div className="h-1 bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600" />
                ...
                <CardHeader className="space-y-1 text-center pb-3 sm:pb-5 pt-4 sm:pt-7">
                  <motion.div
                    ...
                    className="mx-auto relative mb-5"
                  >
                    <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl bg-gradient-to-br from-indigo-600 to-violet-600 flex items-center justify-center shadow-2xl">
                  ...
                  <motion.div
                    ...
                    transition={{ delay: 0.3 }}
                  >
                    <CardTitle className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">
                      设置新密码
                    </CardTitle>
                ...
                <CardContent className="px-4 sm:px-6 pb-5 sm:pb-7">
                  <Form method="post" className="space-y-4">
                    ...
                    >
                      <Button
                        type="submit"
                        className="w-full h-11 sm:h-12 text-sm font-medium bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
